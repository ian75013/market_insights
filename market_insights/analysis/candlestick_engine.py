...
def annotate_candlesticks(df: pd.DataFrame) -> list[dict]:
    ...
    for i in range(len(df)):
        ...
        if i > 0:
            if low_price > ph:
                gap_pct = round((low_price - ph) / ph * 100, 2)
                signals.append(
                    {
                        "type": "gap_up",
                        "label": f"Gap haussier +{gap_pct}%",
                        "severity": "bullish",
                        "value": gap_pct,
                    }
                )
            if high_price < pl:
                gap_pct = round((pl - high_price) / pl * 100, 2)
                signals.append(
                    {
                        "type": "gap_down",
                        "label": f"Gap baissier -{gap_pct}%",
                        "severity": "bearish",
                        "value": gap_pct,
                    }
                )
        ...
        if sma20 > 0 and abs(close_price - sma20) / max(close_price, 1e-9) <= 0.008:
            signals.append(
                {
                    "type": "pullback_sma20",
                    "label": "Pullback SMA 20",
                    "severity": "neutral",
                    "value": round(sma20, 2),
                }
            )
        if sma50 > 0 and abs(close_price - sma50) / max(close_price, 1e-9) <= 0.008:
            signals.append(
                {
                    "type": "pullback_sma50",
                    "label": "Pullback SMA 50",
                    "severity": "neutral",
                    "value": round(sma50, 2),
                }
            )
        ...
        if (
            ...
            signals.append(
                {"type": "hammer", "label": "Marteau", "severity": "bullish"}
            )
        ...
        if i >= 2:
            ...
            if (
                p2c < p2o
                and abs(pc - po) / max(abs(p2c - p2o), 1e-9) < 0.3
                and close_price > open_price
                and close_price > (p2o + p2c) / 2
            ):
                signals.append(
                    {
                        "type": "morning_star",
        ...
        if i >= 20:
            ...
            if vol_avg > 0 and vol >= 1.8 * vol_avg:
                signals.append(
                    {
                        "type": "volume_spike",
                        "label": f"Volume ×{vol / vol_avg:.1f}",
                        "severity": "neutral",
                        "value": round(vol / vol_avg, 1),
                    }
                )
