    ...
    def fetch(self, ticker: str, max_items: int = 10) -> list[dict]:
        ...
        for feed_url_template in self.RSS_FEEDS:
            try:
                ...
                xml_text = self.get_text(url, cache_key=f"rss:{ticker}:{feed_url_template[:30]}")
    ...
    def _parse_rss(xml_text: str, max_items: int = 10) -> list[dict]:
        ...
        for item_el in root.findall(".//item")[:max_items]:
            items.append({
                ...
                "content": (item_el.findtext("description", default="") or "")[:400],
    ...
    def fetch(self, ticker: str, max_items: int = 10) -> list[dict]:
        ...
        try:
            ...
            if conn.available():
                ...
                if items:
                    logger.info("News for %s resolved via Alpha Vantage (%d items)", ticker, len(items))
                    return items
        except Exception as exc:
