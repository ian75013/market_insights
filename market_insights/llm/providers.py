...
Supports: OpenAI, Anthropic, Mistral, Groq, Ollama (local), LMStudio (local), fallback (deterministic).

Usage:
    from market_insights.llm.providers import get_llm, list_providers
    llm = get_llm("openai")
    response = llm.generate("Analyse technique AAPL", system="Tu es un analyste financier.")
"""
...
class BaseLLMProvider(ABC):
    ...
    @abstractmethod
    def generate(self, prompt: str, *, system: str = "", temperature: float | None = None, max_tokens: int | None = None) -> LLMResponse: ...
...
class MistralProvider(BaseLLMProvider):
    ...
    DEFAULT_MODELS = ["mistral-large-latest", "mistral-medium-latest", "mistral-small-latest", "open-mistral-nemo"]
    ...
    def generate(self, prompt: str, *, system: str = "", temperature: float | None = None, max_tokens: int | None = None) -> LLMResponse:
        ...
        with httpx.Client(timeout=60) as client:
            resp = client.post(
                ...
                json={"model": model, "messages": messages, "temperature": temperature or settings.llm_temperature, "max_tokens": max_tokens or settings.llm_max_tokens},
            )
...
class GroqProvider(BaseLLMProvider):
    ...
    def generate(self, prompt: str, *, system: str = "", temperature: float | None = None, max_tokens: int | None = None) -> LLMResponse:
        ...
        with httpx.Client(timeout=60) as client:
            ...
            resp.raise_for_status()
            data = resp.json()
        return LLMResponse(text=data["choices"][0]["message"]["content"], model=model, provider="groq", usage=data.get("usage"))
...
class OllamaProvider(BaseLLMProvider):
    ...
    def generate(self, prompt: str, *, system: str = "", temperature: float | None = None, max_tokens: int | None = None) -> LLMResponse:
        ...
        return LLMResponse(
            ...
            model=model,
            provider="ollama",
            usage={"total_duration": data.get("total_duration"), "eval_count": data.get("eval_count")},
        )
...
class LMStudioProvider(BaseLLMProvider):
    ...
    def generate(self, prompt: str, *, system: str = "", temperature: float | None = None, max_tokens: int | None = None) -> LLMResponse:
        ...
        with httpx.Client(timeout=120) as client:
            ...
            resp.raise_for_status()
            data = resp.json()
        return LLMResponse(text=data["choices"][0]["message"]["content"], model=model, provider="lmstudio", usage=data.get("usage"))
