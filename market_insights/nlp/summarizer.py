"""Extractive summarizer + HTML stripper."""
...
_STOP = frozenset(
    "the and or a an of to in for with on at by is it that this "
    "was are be has have had will can may from its as but not "
    "their we he she they our le la les de du des un une et en "
    "à pour par sur est ce qui que dans son sa ses avec ne pas "
    "au aux cette".split()
)
...
def _tok(t: str) -> list[str]:
    return [
        w for w in re.findall(r"\b[a-zà-ÿ]{2,}\b", t.lower())
        if w not in _STOP
    ]
